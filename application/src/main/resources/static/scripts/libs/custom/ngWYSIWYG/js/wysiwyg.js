...
var template = "<div class=\"tinyeditor\">" +
	...
	"<div class=\"tinyeditor-buttons-group\">" +
	    ...
	    "<select class=\"tinyeditor-style\" ng-model=\"field\" ng-options=\"s.value as s.name for s in fields\" ng-change=\"insertField()\"><option value=\"\">Campos ...</option></select>" +
	"</div>" +
...
angular.module('ngWYSIWYG').directive('wysiwygEdit', ['$compile', '$timeout', '$q',
	...
	var linker = function( scope, $element, attrs, ctrl ) {
	    ...
	    */
	    ...
	    scope.fields = [{name: 'Nome', value: '[nome]'}, {name: 'Endereço', value: '[endereco]'}, {name: 'Cidade', value: '[cidade]'}, {name: 'CPF', value: '[cpf]'}, {name: 'Plano', value: '[plano]'}];
	    scope.insertField = function() {
            scope.execCommand('insertHTML', scope.field);
	    }
